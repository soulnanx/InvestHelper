package entropia.app.com.andoidcdb.ui.chart;
        ...
        if (dataLists != null && !dataLists.isEmpty()) {
            for (ArrayList<Double> list : dataLists) {
                for (Double aDouble : list) {
                    if (verticalGridNum < ((int)(aDouble * 1000) + 1)) {
                        verticalGridNum = (int)(aDouble * 1000) + 1;
                    }
                }
            }
        }
        ...
        if (dataLists != null && !dataLists.isEmpty()) {
            ...
            for (int k = 0; k < dataLists.size(); k++) {
                ...
                for (int i = 0; i < dataLists.get(k).size(); i++) {
                    ...
                    int y = yCoordinateList.get(verticalGridNum - (int)(dataLists.get(k).get(i) * 1000));
                    if (i > drawDotSize - 1) {
                        //도트리스트를 추가한다.
...
//        mViewHeight = MeasureSpec.getSize(measureSpec);
    ...
    class Dot {
        ...
        Dot(int x, int y, int targetX, double targetY, Double data, int linenumber) {
            this.x = x;
            this.y = y;
        ...
        Dot setTargetData(int targetX, double targetY, Double data, int linenumber) {
            ...
            this.data = new BigDecimal(data, MathContext.DECIMAL64).doubleValue();
            this.linenumber = linenumber;
            return this;
        }
        ...
        private int updateSelf(int origin, double target, int velocity) {
            if (origin < target) {
                origin += velocity;
            } else if (origin > target) {
                origin -= velocity;
